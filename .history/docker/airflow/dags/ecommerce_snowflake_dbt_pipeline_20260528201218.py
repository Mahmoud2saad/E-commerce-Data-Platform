...
with DAG(
    ...
    description="Run the e-commerce Snowflake dbt pipeline from silver to gold validation.",
    ...
    run_silver = dbt_task(
        ...
        command="dbt run --select path:models/silver",
    )

    run_intermediate = dbt_task(
        task_id="run_intermediate_models",
        command="dbt run --select path:models/intermediate",
    )

    run_gold = dbt_task(
        task_id="run_gold_models",
        command="dbt run --select path:models/gold",
    )

    run_validations = dbt_task(
        task_id="run_validation_models",
        command=(
            "dbt run --select "
            "silver_validation_summary "
            "stream_silver_validation_summary "
            "cdc_silver_validation_summary "
            "intermediate_unified_validation_summary "
