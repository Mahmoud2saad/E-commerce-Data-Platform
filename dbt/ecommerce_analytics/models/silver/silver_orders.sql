...
cleaned as (
    select
        *,
        to_timestamp_ntz('2026-05-20 23:59:59') as batch_cutoff_timestamp,

        (order_approved_at_shifted > to_timestamp_ntz('2026-05-20 23:59:59'))
            as is_approved_after_batch_cutoff,

        (order_delivered_carrier_timestamp_shifted > to_timestamp_ntz('2026-05-20 23:59:59'))
            as is_carrier_after_batch_cutoff,

        (order_delivered_customer_timestamp_shifted > to_timestamp_ntz('2026-05-20 23:59:59'))
            as is_customer_delivery_after_batch_cutoff,

        (order_estimated_delivery_timestamp_shifted > to_timestamp_ntz('2026-05-20 23:59:59'))
            as is_estimated_delivery_after_batch_cutoff,

        (order_delivered_carrier_timestamp_shifted < order_purchase_timestamp_shifted)
            as has_carrier_before_purchase,

        (order_approved_at_shifted is not null
            and order_delivered_carrier_timestamp_shifted is not null
            and order_delivered_carrier_timestamp_shifted < order_approved_at_shifted)
            as has_carrier_before_approved,

        (order_delivered_customer_timestamp_shifted is not null
            and order_delivered_customer_timestamp_shifted < order_purchase_timestamp_shifted)
            as has_customer_delivery_before_purchase,

        (order_delivered_customer_timestamp_shifted is not null
            and order_delivered_carrier_timestamp_shifted is not null
            and order_delivered_customer_timestamp_shifted < order_delivered_carrier_timestamp_shifted)
            as has_customer_delivery_before_carrier,

        (order_estimated_delivery_timestamp_shifted < order_purchase_timestamp_shifted)
            as has_estimated_delivery_before_purchase,

        (order_status = 'delivered'
            and order_delivered_customer_timestamp_shifted is null)
            as is_missing_delivery_date,

        (order_status = 'delivered'
            and order_delivered_customer_timestamp_shifted is null
            and order_estimated_delivery_timestamp_shifted is not null
            and order_estimated_delivery_timestamp_shifted <= to_timestamp_ntz('2026-05-20 23:59:59'))
            as is_delivery_date_imputed,

        (order_status = 'canceled'
            and order_delivered_customer_timestamp_shifted is not null)
            as is_canceled_with_delivery_date,

        datediff(day, order_purchase_timestamp_shifted, to_timestamp_ntz('2026-05-20 23:59:59'))
            as order_age_days
    from orders
),
...
business_rules as (
    select
        *,
        ...
        case
            when is_approved_after_batch_cutoff then null
            else order_approved_at_shifted
        end as clean_order_approved_timestamp,

        case
            when is_estimated_delivery_after_batch_cutoff then null
            else order_estimated_delivery_timestamp_shifted
        end as clean_estimated_delivery_timestamp,

        case
            when has_carrier_before_purchase
                or is_carrier_after_batch_cutoff
                then null
            else order_delivered_carrier_timestamp_shifted
        ...
        *,
        case
            ...
            when has_customer_delivery_before_purchase
                or has_customer_delivery_before_carrier
                or is_customer_delivery_after_batch_cutoff
                then null
            else order_delivered_customer_timestamp_shifted
